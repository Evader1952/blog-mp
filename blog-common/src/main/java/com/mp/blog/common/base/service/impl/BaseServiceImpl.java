package com.mp.blog.common.base.service.impl;

import com.google.common.base.Preconditions;
import com.mp.blog.common.base.dao.jpa.BaseRepository;
import com.mp.blog.common.base.service.BaseService;
import com.mp.blog.common.model.Identifiable;
import com.mp.blog.common.utils.BeanUtils;
...
public abstract class BaseServiceImpl<T extends Identifiable<ID>, ID extends Serializable> implements BaseService<T, ID> {
    ...
    public void insertBatch(List<T> list) {
        Preconditions.checkArgument(DataUtils.isNotEmpty(list), "批量插入传入列表为空");
    ...
    public T updateById(T entity) {
        Preconditions.checkArgument(DataUtils.isNotEmpty(entity), "待修改的数据不能为空");
        Preconditions.checkArgument(DataUtils.isNotEmpty(entity.getId()), "待修改的数据不能为空");
        return getRepository().saveAndFlush(entity);
    }
    ...
    protected Specification<T> makeSimpleSpecification(final T query) {
        Specification<T> specification = (root, criteriaQuery, criteriaBuilder) -> {
            try {
                Map<String, Object> map = BeanUtils.toMap(query);
