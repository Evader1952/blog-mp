...
import com.mp.blog.common.entity.CalculateDetail;
import com.mp.blog.common.entity.CalculateRepaymentDetail;
import com.mp.blog.common.entity.DealDetail;
...
public class MoneyUtil {
     ...
     * @return
     */
    public static CalculateRepaymentDetail calculateRepayment(String payAmt, String loanAmt, Integer fqNum, String rate, Integer type, String pkgTyp, String bonusAmount) {
        ...
        if (type.equals(0)) {
            ...
            if ("01".equals(pkgTyp)) {
                //月返红包金额 = 每期还款本息
                BigDecimal redPackAmount = new BigDecimal(bonusAmount);
                //1+费率
                BigDecimal allRate = new BigDecimal(rate).add(new BigDecimal(1));
                //借款本息= 结算金额*（1+费率）
                BigDecimal allAmount = new BigDecimal(loanAmt).multiply(allRate);
                //每期还款本金=红包/(1+费率) （四舍五入到分）
                each = redPackAmount.divide(allRate, 2, BigDecimal.ROUND_DOWN).setScale(2, BigDecimal.ROUND_DOWN);
                //每期还款利息=红包-本金
                eachInterest = redPackAmount.subtract(each);
                //每期还款本息=每期还款利息+每期还款本金
                eachTotal = each.add(eachInterest);
                //特殊还款本金（首期/尾期）=直降金额-(分期期数-1)*每期还款本金
                special = new BigDecimal(loanAmt).subtract(fqNumNoLast.multiply(each));
                //特殊还款本息（首期/尾期）=借款本息-(分期期数-1)*月反红包金额
                specialTotal = allAmount.subtract(fqNumNoLast.multiply(redPackAmount));
                //特殊还款利息=特殊还款本息 -特殊还款本金
        ...
        if (type.equals(1)) {
            ...
            if ("01".equals(pkgTyp)) {
                ...
                special = new BigDecimal(loanAmt).subtract(fqNumNoLast.multiply(each));
                //特殊还款本息（首期/尾期）=借款本息-(分期期数-1)*月反红包金额
                specialTotal = new BigDecimal(payAmt).subtract(fqNumNoLast.multiply(redPackAmount));
                //特殊还款利息（首期/尾期） =特殊还款本息 -特殊还款本金
                specialInterest = specialTotal.subtract(special);
                //利息=每期还款利息×(分期期数-1)+特殊还款利息（首期/尾期）
                interest = eachInterest.multiply(fqNumNoLast).add(specialInterest);
